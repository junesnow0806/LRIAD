...
for dataset, ax in zip(datasets, axes):
    ...
    ax.plot(epsilon_range, gamma_appr_list, label="v'_tar", color="blueviolet", marker='.')
    ax.plot(epsilon_range, gamma_top_list, label="v_top", color="red", marker='.')
    ax.plot(epsilon_range, gamma_adv_list, label="v_adv", color="limegreen", marker='.')

lines, labels = fig.axes[-1].get_legend_handles_labels()
fig.legend(lines, labels, bbox_to_anchor=(0.5, -0.08), ncol=5, loc="lower center", columnspacing=5.0)
plt.savefig(f"figs/gamma_epsilon.png", bbox_inches='tight')
